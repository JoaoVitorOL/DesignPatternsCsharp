...
Todo executável possui um único ponto de entrada. Em projetos modernos, o compilador pode sintetizá-lo a partir de top-level statements; na forma explícita, ele procura uma assinatura válida de `Main`. O ponto de entrada pode receber argumentos, devolver um código de saída e ser assíncrono (`Task` ou `Task<int>`). Bibliotecas de classes, por outro lado, não precisam de `Main` porque são carregadas por outro processo.
...
> **Regra de estrutura:** somente um arquivo da compilação pode conter top-level statements, e eles devem aparecer antes das declarações de namespace e tipo daquele arquivo. Para aplicações maiores, mova regras para tipos e métodos testáveis; deixe o ponto de entrada responsável sobretudo por configuração e composição.
...
Este capítulo explica como o compilador resolve nomes e como o código organiza tipos sem confundir namespace, pasta, projeto e assembly. As diretivas `using` reduzem repetição, mas não adicionam dependências. Dominar essa distinção evita imports “mágicos”, ambiguidades e estruturas de solução que parecem organizadas apenas pela aparência.
...
Use a forma file-scoped quando o arquivo pertence a um único namespace; ela reduz indentação sem mudar a semântica. Evite namespaces profundos que apenas repetem cada pasta e não use o namespace como substituto para uma fronteira de assembly ou para encapsulamento.

> **Referência oficial:** [Declare namespaces to organize types](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/types/namespaces)
...
// C# 12+: alias para qualquer tipo, inclusive tuple, array e ponteiro
using ParDeTexto = (string Chave, string Valor);
...
Um `using` apenas permite usar nomes mais curtos; ele não adiciona referência de projeto, não carrega assembly e não copia código. Se houver ambiguidade, use alias ou nome totalmente qualificado. Coloque `global using` em um arquivo previsível ou deixe o SDK gerar os imports implícitos, evitando espalhar dependências invisíveis sem critério.

Diretivas `using` são escopo de nomes em compilação, não gerenciamento de recursos; o `using` statement da seção 7.7 é outra construção.

> **Referência oficial:** [Using directive](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/keywords/using-directive)
...
Tipos determinam quais valores e operações são válidos; variáveis dão nome a locais que guardam ou referenciam esses valores. Aqui entram cópia e identidade, conversões, nulabilidade, inferência, constantes, boxing, literais e descartes. Esse modelo é a base para prever mutabilidade, alocação, passagem de parâmetros e erros de runtime.
...
O **escopo** determina onde o nome pode ser usado; o **tempo de vida** determina por quanto tempo seu armazenamento existe. Parâmetros e variáveis locais precisam obedecer à análise de atribuição definida: o compilador impede a leitura de um local antes de provar que ele recebeu valor. Campos, em contraste, recebem o valor padrão do tipo antes da execução do construtor.
...
Prefira o menor escopo que preserve a clareza. Um nome próximo do uso reduz estado mutável e facilita raciocinar sobre caminhos de execução. Para valores que não devem mudar, use uma variável local sem reatribuição por disciplina ou modele a restrição no tipo/campo apropriado; C# não possui um modificador geral de local imutável equivalente a `const` para qualquer tipo.
...
Anote o contrato com honestidade: aceite `T?` quando ausência for válida, retorne `T?` quando o resultado puder faltar e valide entradas obrigatórias na fronteira. Isso permite ao fluxo anulável do compilador acompanhar testes como `is not null` e reduzir avisos sem espalhar `!`.
...
`var` só pode ser usado em contextos locais nos quais o inicializador permite inferir um tipo: variáveis locais, inicializadores de `for`, recursos de `using` e variáveis de iteração. Ele não pode declarar campo, parâmetro ou retorno de método. Também não aceita uma declaração sem valor inicial nem o literal `null` sozinho, pois nesses casos falta um tipo-alvo.
...
`var` é necessário para armazenar tipos anônimos e costuma melhorar a leitura quando o tipo aparece claramente à direita, como `new Dictionary<string, int>()`. Prefira o tipo explícito quando ele comunica uma abstração relevante ou quando a chamada não revela o retorno. A escolha é de legibilidade; não altera desempenho nem o tipo gerado.

> **Referência oficial:** [Implicitly typed local variables](https://learn.microsoft.com/en-us/dotnet/csharp/programming-guide/classes-and-structs/implicitly-typed-local-variables)
...
Os dois modificadores impedem reatribuição em momentos diferentes. `const` representa uma expressão constante conhecida pelo compilador e seu valor é substituído no código consumidor. `readonly` atua sobre um campo: permite que cada instância receba um valor no construtor e bloqueia novas atribuições depois. Para um valor compartilhado calculado em runtime, use `static readonly`.
...
`readonly` não torna o objeto referenciado imutável, como a lista do exemplo demonstra. Além disso, alterar um `public const` em uma biblioteca não atualiza consumidores já compilados; eles precisam ser recompilados para incorporar o novo valor. Por isso valores públicos que podem evoluir normalmente são expostos como propriedades ou campos `static readonly`.

> **Referências oficiais:** [`const` keyword](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/keywords/const), [`readonly` keyword](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/keywords/readonly)
...
### 3.8 Literais, target typing e descartes

[⬆️ Voltar ao Sumário](#sumário)

Um **literal** escreve um valor diretamente no código. Prefixos e sufixos deixam tipo e base numérica explícitos; separadores `_` melhoram a leitura sem alterar o valor. O contexto também pode fornecer o tipo esperado, mecanismo chamado *target typing*: ele aparece em `new()`, `default`, expressões condicionais, lambdas e vários recursos modernos.

```csharp
int binario = 0b_1101_0010;
int hexadecimal = 0xFF_A0;
long populacao = 8_100_000_000L;
decimal preco = 19.90m;           // m/M seleciona decimal
double constante = 6.022e23;     // notação científica

List<string> nomes = new();       // o lado esquerdo fornece List<string>
DateOnly data = new(2026, 7, 21); // o construtor é inferido como DateOnly
int valorInicial = default;       // default(int), isto é, zero

_ = int.TryParse("42", out int numero); // descarta o bool, mantém numero
var (nome, _) = ("Ana", 28);            // ignora o segundo componente
```

O descarte `_` comunica que um resultado existe, mas não interessa naquele ponto. Ele evita variáveis artificiais em desconstruções, padrões, parâmetros de lambda e argumentos `out`. Não use descartes para esconder falhas que precisam ser tratadas: ignorar o `bool` de `TryParse`, como no exemplo, só é correto quando um valor inválido realmente não muda o fluxo. `default` também exige cuidado: o valor padrão de um tipo de referência é `null`, e uma estrutura pode precisar de invariantes que sua representação zerada não satisfaz.

> **Referências oficiais:** [Built-in types](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/builtin-types/built-in-types), [Target-typed `new`](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/operators/new-operator), [Default values](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/builtin-types/default-values), [Discards](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/functional/discards)
...
Texto parece simples, mas envolve imutabilidade, Unicode, cultura, comparação e alocação. Esta parte diferencia conteúdo de representação e apresenta as ferramentas mais comuns da BCL para construir, comparar, formatar e transformar strings. A meta é escrever código correto tanto para identificadores de máquina quanto para texto destinado a pessoas.
...
Em C#, `string` é um tipo de referência (`System.String`), mas se comporta de forma especial: é **imutável** e possui otimizações de *interning* mantidas pelo runtime.

Imutabilidade significa que nenhum membro altera os caracteres de uma instância existente: operações como `Replace`, `Trim` e `ToUpper` devolvem outra string. A variável ainda pode ser reatribuída para essa nova instância. Literais iguais podem ser internados pelo runtime, mas a lógica do programa nunca deve depender de igualdade de referência entre strings.
...
Use comparação **ordinal** para identificadores, protocolos, caminhos já normalizados e chaves internas; use comparação cultural para texto linguístico apresentado a pessoas. Uma string é uma sequência UTF-16, portanto `Length` conta unidades de código, não necessariamente caracteres percebidos pelo usuário.

> **Referências oficiais:** [Strings and string literals](https://learn.microsoft.com/en-us/dotnet/csharp/programming-guide/strings/), [Best practices for comparing strings](https://learn.microsoft.com/en-us/dotnet/standard/base-types/best-practices-strings)
...
Para poucas concatenações em uma única expressão, `+` ou interpolação continuam claros e o compilador pode otimizá-los. Use `StringBuilder` quando a montagem é incremental, especialmente em laços ou quando o tamanho final é desconhecido; meça antes de trocar legibilidade por uma otimização sem impacto.
...
Literais de string controlam duas coisas diferentes: como o texto é escrito no fonte e como valores são formatados. Interpolação (`$`) insere expressões; verbatim (`@`) reduz escapes; raw strings (`"""`) preservam aspas e múltiplas linhas. Especificadores depois de `:` e alinhamento depois de `,` usam o sistema de formatação do tipo.
...
string coluna = $"{total,12:N2}"; // alinhamento mínimo 12; formato numérico N2

// Literal UTF-8 (C# 11+): ReadOnlySpan<byte>, útil em protocolos e APIs de bytes
ReadOnlySpan<byte> cabecalho = "OK\r\n"u8;
```
...
A interpolação usa a cultura atual quando materializada como `string`, salvo quando uma API ou `IFormatProvider` explícito determina outra cultura. Em logging estruturado, não substitua templates do logger por interpolação sem avaliar a perda de propriedades estruturadas. Interpolated string handlers permitem que APIs evitem trabalho e alocações em cenários específicos; são uma otimização de biblioteca, não uma razão para abandonar a sintaxe simples.

> **Referências oficiais:** [String interpolation](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/tokens/interpolated), [Raw string literals](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/tokens/raw-string), [Built-in reference types — UTF-8 string literals](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/builtin-types/reference-types#the-string-type)
...
Antes de escolher um método, defina se a operação é **ordinal** ou **linguística**, se aceita `null`, se deve alocar outra string e se a entrada é confiável. Muitos overloads de `Contains`, `StartsWith`, `EndsWith`, `IndexOf` e `Equals` aceitam `StringComparison`; declarar a intenção evita resultados diferentes entre culturas.
...
`Substring`, `Split`, `Replace`, mudança de caixa e `Trim` normalmente produzem novas strings ou coleções. Em parsing de alto volume, `ReadOnlySpan<char>` pode representar uma fatia sem criar substring. Não use `ToLower()` como preparação genérica para comparar: isso aloca e pode introduzir regras culturais indesejadas; passe o `StringComparison` correto diretamente.

| Intenção | API preferida |
|---|---|
| validar ausência | `string.IsNullOrWhiteSpace` |
| comparar chave interna | `string.Equals(a, b, StringComparison.Ordinal)` |
| procurar ignorando caixa | `Contains(valor, StringComparison.OrdinalIgnoreCase)` |
| unir sequência | `string.Join` |
| converter entrada sem exceção esperada | `TryParse` do tipo de destino |

> **Referência oficial:** [Common string operations](https://learn.microsoft.com/en-us/dotnet/csharp/programming-guide/strings/)
...
Modificadores de acesso definem quem pode depender de um tipo ou membro e, portanto, quais decisões viram contrato. Este capítulo trata acessibilidade como ferramenta de design: começar fechado, abrir de forma intencional e distinguir colaboração no tipo, na hierarquia, no arquivo, no assembly e fora dele.
...
Propriedades oferecem sintaxe de campo com contrato de membros e acessores. Elas permitem controlar leitura, escrita, inicialização, validação e valores calculados sem expor diretamente o armazenamento. O capítulo também mostra quando uma propriedade deixa de ser uma consulta barata e por que efeitos colaterais surpreendentes devem permanecer em métodos.
...
Um membro de instância opera sobre um objeto específico; um membro `static` pertence ao tipo e pode ser usado sem criar objeto. Classes estáticas servem como agrupadores de operações sem estado por instância e não podem ser instanciadas nem herdadas. Campos estáticos existem uma vez por tipo carregado naquele contexto de execução, não uma vez por máquina ou por aplicação distribuída.
...
O contador didático não é thread-safe: duas threads podem perder incrementos. Estado estático mutável deve ter política explícita de sincronização, inicialização e descarte. Prefira métodos estáticos puros, constantes e dados imutáveis; não use `static` como atalho para transformar dependências e estado global em acesso invisível.

> **Referências oficiais:** [`static` keyword](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/keywords/static), [Static classes and static class members](https://learn.microsoft.com/en-us/dotnet/csharp/programming-guide/classes-and-structs/static-classes-and-static-class-members)
...
`sealed` pode fechar uma classe inteira ou interromper a sobrescrita de um membro que já foi herdado. A decisão comunica que a implementação não oferece um ponto de extensão seguro naquela direção. Isso é especialmente útil para tipos com invariantes estritas, objetos de valor e implementações que devem ser consumidas por composição ou interface.
...
Não sele classes automaticamente apenas por possível otimização: o ganho de runtime depende do contexto e do compilador. Se consumidores precisam substituir comportamento, exponha um contrato apropriado; se não existe contrato de herança documentado, fechar a classe reduz a superfície que precisa permanecer compatível.

> **Referência oficial:** [`sealed` modifier](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/keywords/sealed)
...
Uma classe abstrata define uma base incompleta: não pode ser instanciada, mas pode possuir campos, estado, construtores, membros concretos, virtuais e abstratos. Um membro abstrato declara o contrato sem corpo e obriga a primeira classe concreta da hierarquia a implementá-lo. A base ainda controla a ordem de inicialização e pode proteger comportamento comum.
...
Escolha uma classe abstrata quando os tipos compartilham estado ou implementação e formam uma relação de especialização estável. Use interface quando o objetivo principal é declarar uma capacidade que tipos não relacionados podem implementar. Como C# permite apenas uma classe base direta, uma hierarquia abstrata é uma decisão mais restritiva que implementar interfaces.

> **Referência oficial:** [`abstract` keyword](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/keywords/abstract)
...
O despacho de um membro virtual usa o tipo real do objeto. Já um membro ocultado com `new` é escolhido a partir do tipo estático da expressão, como `a.Respirar()` demonstra. Um override pode chamar `base.Membro()` para reutilizar parte do comportamento e pode devolver um tipo de retorno mais derivado quando a regra de retorno covariante permitir.

Evite chamar membros virtuais em construtores: o override pode executar antes de a parte derivada estar completamente inicializada. Uma API que marca um membro como virtual assume um compromisso de compatibilidade com implementações externas.

> **Referências oficiais:** [`virtual`](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/keywords/virtual), [`override`](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/keywords/override), [`new` modifier](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/keywords/new-modifier)
...
`this` representa a instância atual e desambigua membros de parâmetros, encadeia construtores da mesma classe ou é passado como argumento. `base` acessa a implementação da classe base e seleciona seu construtor. Nenhuma das duas palavras cria outro objeto; elas tornam explícito qual instância ou nível da hierarquia participa da operação.
...
public class Pessoa
{
    ...
    public Pessoa ComIdade(int idade)
    {
        ...
        this.Idade = idade; // 'this' deixa explícito o membro da instância
        return this;
    }
}
...
Um inicializador de construtor usa **ou** `this(...)` **ou** `base(...)`. A cadeia precisa terminar em um construtor que alcance a base; use-a para centralizar invariantes, não para criar ciclos. Em structs, `this` também representa o valor atual, mas as regras de atribuição e cópia seguem a semântica de tipo de valor.

> **Referências oficiais:** [`this` keyword](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/keywords/this), [`base` keyword](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/keywords/base)
...
`as` só funciona com conversões de referência, anuláveis e boxing compatíveis; ele não substitui conversões numéricas. Quando você precisa testar e usar o valor, `is Tipo variavel` evita separar o cast da verificação e participa da análise de fluxo do compilador.
...
O `using` de recursos é açúcar sintático para garantir descarte em um `finally`. Ele funciona com `IDisposable` e também com o padrão de descarte reconhecido pela linguagem; `await using` aguarda `DisposeAsync` de `IAsyncDisposable`. O descarte acontece mesmo se o corpo lançar exceção e, para vários recursos, ocorre na ordem inversa da aquisição.
...
Descarte não significa necessariamente “fechar para sempre”: a implementação define a operação. Também não capture um recurso descartável além do escopo do `using`. A regra de ownership é essencial: quem cria ou recebe a responsabilidade declarada pelo contrato deve descartar, sem descarte duplo ou uso posterior.

> **Referências oficiais:** [`using` statement](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/statements/using), [Implement a Dispose method](https://learn.microsoft.com/en-us/dotnet/standard/garbage-collection/implementing-dispose), [Implement `DisposeAsync`](https://learn.microsoft.com/en-us/dotnet/standard/garbage-collection/implementing-disposeasync)
...
Normalmente, C# passa argumentos **por valor**: copia o valor do argumento; para uma classe, esse valor copiado é a referência. Os modificadores desta seção passam o próprio local de armazenamento. Por isso eles fazem parte da assinatura e o chamador normalmente também escreve `ref`, `out` ou `in`, tornando o compartilhamento explícito.
...
`ref` também aparece em locais e retornos por referência, permitindo operar sobre o armazenamento original, e `ref readonly` expõe uma referência sem permitir escrita. `scoped` restringe o escape de referências e ajuda a provar que dados ligados à stack não sobrevivem além do escopo seguro. Essas APIs exigem documentação clara de lifetime e aliasing.

> **Referências oficiais:** [Method parameters](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/keywords/method-parameters), [Safe access to memory with `ref`](https://learn.microsoft.com/en-us/dotnet/csharp/advanced-topics/performance/)
...
`nameof` é resolvido na compilação e acompanha renomeações; `typeof` produz um `System.Type` para um tipo conhecido estaticamente; `GetType()` inspeciona o tipo de runtime de uma instância. `default` depende do tipo alvo e pode produzir `null`, portanto não promete um objeto válido.
...
Todas as partes de um tipo `partial` são combinadas na mesma compilação e precisam concordar em aspectos como namespace e parâmetro de tipo. O recurso é especialmente útil para código gerado; não o use apenas para espalhar uma classe grande sem separar responsabilidades.
...
Um método iterator retorna `IEnumerable<T>`/`IEnumerator<T>` ou suas formas assíncronas e contém `yield return` ou `yield break`. O compilador transforma o método em uma máquina de estados. Chamar o método geralmente cria o enumerável; o corpo começa a executar quando o consumidor pede o primeiro elemento.
...
Consequências práticas: enumerar novamente pode repetir trabalho e efeitos colaterais; exceções podem surgir durante a enumeração, não na criação da sequência; abandonar um `foreach` descarta o enumerador e executa os blocos `finally` do iterator. Use `yield break` para encerrar e não retorne recursos cujo lifetime termine antes do consumo.
...
`lock` protege uma região síncrona usando o mesmo objeto de sincronização para todos os acessos ao estado relacionado. A exclusão mútua só funciona quando cada caminho respeita o mesmo protocolo; proteger a escrita e deixar a leitura sem coordenação ainda pode produzir estado inconsistente.
...
Use um gate privado e dedicado. Não bloqueie `this`, strings ou objetos publicamente acessíveis. Evite I/O, callbacks e trabalho demorado dentro do bloco. `await` não é permitido no corpo de `lock`; para coordenação assíncrona, uma opção comum é `SemaphoreSlim.WaitAsync`. Em C# 13/.NET 9+, `System.Threading.Lock` possui tratamento específico pela linguagem.
...
Um método `async` sem `await` executa sincronamente e normalmente indica implementação incompleta ou wrapper desnecessário. Prefira os sufixos `Async`, propague `CancellationToken` quando a operação puder demorar e evite `async void`, exceto nos contratos de event handler que o exigem.
...
Esses recursos fornecem garantias de inicialização, não validação de conteúdo: `Nome = ""` ainda satisfaz `required`, e uma referência exigida ainda pode receber `null` se avisos forem ignorados. Use construtores ou validação explícita quando o objeto só puder existir com invariantes fortes.
...
Controle de fluxo decide quais instruções executam, quantas vezes e com qual valor resultante. Este capítulo conecta condicionais, patterns, laços e operadores às regras de precedência, curto-circuito e overflow. O objetivo não é apenas compilar decisões, mas torná-las legíveis e previsíveis para quem mantém o código.
...
`if` avalia uma expressão do tipo `bool`; C# não converte automaticamente números, strings ou objetos em verdadeiro/falso. Os ramos são testados em ordem e apenas o primeiro ramo compatível de uma cadeia é executado. Para invariantes simples, retornos antecipados (*guard clauses*) costumam reduzir aninhamento e deixar o caminho principal visível.
...
Use chaves mesmo quando o corpo possui uma linha se a equipe prioriza segurança durante manutenção; o compilador não exige, mas adicionar outra instrução sem as chaves pode mudar o fluxo. O operador ternário é uma expressão e deve permanecer curto. Para classificações por forma, tipo ou muitos casos, `switch` e pattern matching podem comunicar melhor a intenção.

> **Referência oficial:** [Selection statements](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/statements/selection-statements)
...
O `switch` statement executa instruções; a switch expression produz um valor. Ambos aceitam patterns e guards, e os casos são considerados de cima para baixo. Um braço mais abrangente antes de outro específico pode tornar o segundo inalcançável, diagnóstico que o compilador ajuda a detectar.
...
Uma switch expression que recebe um valor não coberto lança `SwitchExpressionException` em runtime; inclua descarte (`_`) quando o domínio não é comprovadamente exaustivo. Para enums, lembre que qualquer valor do tipo subjacente pode chegar por cast ou desserialização. Guards com `when` devem complementar o pattern, não esconder efeitos colaterais difíceis de prever.

> **Referências oficiais:** [`switch` statement](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/statements/selection-statements#the-switch-statement), [`switch` expression](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/operators/switch-expression)
...
Laços repetem trabalho, mas cada forma expressa um contrato diferente. `for` expõe inicialização, condição e avanço; `foreach` usa o padrão de enumeração; `while` testa antes; `do` testa depois. A condição deve convergir ou existir uma forma intencional de cancelamento, saída ou limite.
...
Não altere estruturalmente uma coleção comum durante seu `foreach`; muitos enumeradores detectam a mudança e lançam `InvalidOperationException`. Para sequências assíncronas, use `await foreach`. Em processamento potencialmente infinito, propague `CancellationToken`. Também avalie a complexidade: um laço aninhado ou uma busca repetida pode dominar o custo mesmo quando cada linha parece barata.

> **Referências oficiais:** [Iteration statements](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/statements/iteration-statements), [`foreach` statement](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/statements/iteration-statements#the-foreach-statement)
...
O tipo dos operandos determina qual operador existe e qual resultado é produzido. Promoções numéricas, operadores anuláveis “levantados”, sobrecarga definida pelo usuário e curto-circuito fazem duas expressões visualmente parecidas terem custos e efeitos diferentes. Consulte a precedência, mas use parênteses quando a intenção não for imediata.
...
### 8.5 Operadores definidos pelo usuário
...
public readonly record struct Distancia(double Metros) : IComparable<Distancia>
{
    public static Distancia operator +(Distancia a, Distancia b) =>
        new(a.Metros + b.Metros);

    public static Distancia operator -(Distancia a, Distancia b) =>
        new(a.Metros - b.Metros);

    public static bool operator <(Distancia a, Distancia b) => a.Metros < b.Metros;
    public static bool operator >(Distancia a, Distancia b) => a.Metros > b.Metros;
...
Métodos transformam intenção em contratos chamáveis: recebem dados, produzem resultados e tornam efeitos visíveis. Aqui você aprende assinaturas, parâmetros, sobrecarga, extensões, funções locais e retornos por referência. Também aparecem decisões de evolução de API, como defaults incorporados no chamador e ambiguidades introduzidas por novas sobrecargas.
...
Um método possui nome, tipo de retorno, lista de parâmetros, parâmetros de tipo genérico, modificadores e corpo. A assinatura é o contrato mecânico que o compilador usa para chamadas; nomes claros e tipos específicos são a documentação mais próxima do uso. Métodos devem preservar as invariantes do tipo e deixar efeitos colaterais importantes visíveis.
...
// Quantidade variável de argumentos com params
...
Argumentos nomeados melhoram chamadas com vários valores semelhantes, mas renomear um parâmetro público pode quebrar código-fonte consumidor. Parâmetros opcionais precisam vir depois dos obrigatórios e seu default é incorporado no chamador. `params` deve ser o último parâmetro; C# 13 ampliou os tipos de coleção reconhecidos além de arrays.

> **Referências oficiais:** [Methods](https://learn.microsoft.com/en-us/dotnet/csharp/methods), [Named and optional arguments](https://learn.microsoft.com/en-us/dotnet/csharp/programming-guide/classes-and-structs/named-and-optional-arguments), [Method parameters](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/keywords/method-parameters)
...
Um membro de instância aplicável tem precedência sobre uma extensão de mesmo nome. A resolução também depende dos namespaces importados e ocorre em compilação; a extensão não altera o tipo original nem participa de despacho virtual. Coloque extensões em namespaces previsíveis e mantenha o receptor como primeiro conceito da operação.

> **Referências oficiais:** [Extension methods](https://learn.microsoft.com/en-us/dotnet/csharp/programming-guide/classes-and-structs/extension-methods), [Extension members](https://learn.microsoft.com/en-us/dotnet/csharp/programming-guide/classes-and-structs/extension-methods#extension-members)
...
Sobrecarga oferece o mesmo conceito com formas de entrada diferentes. O compilador reúne os candidatos visíveis, verifica conversões aplicáveis e escolhe o “melhor” segundo regras formais. Conversões implícitas, argumentos opcionais, `params`, generics e métodos de extensão participam dessa resolução; adicionar uma nova sobrecarga pode tornar uma chamada antes válida ambígua.
...
**Como interpretar o exemplo:** Sobrecarga funciona bem quando todas as assinaturas representam a mesma operação com entradas diferentes. Quando os métodos passam a realizar tarefas semanticamente distantes, nomes diferentes comunicam melhor. Evite pares que aceitam tipos facilmente confundidos ou `null` de forma ambígua; teste as chamadas no ponto de consumo da API.

> **Referências oficiais:** [Member function signature](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/language-specification/basic-concepts#715-signatures-and-overloading), [Method overloading](https://learn.microsoft.com/en-us/dotnet/standard/design-guidelines/member-overloading)
...
Retornos por referência expõem um local, não apenas uma cópia do valor. O compilador aplica regras de escape para impedir referências a variáveis que já deixaram de existir, mas o autor ainda precisa documentar mutabilidade, aliasing e invalidação causada por mudanças na coleção subjacente.
...
Enums substituem números mágicos por um vocabulário finito e tipado, mas continuam apoiados em valores integrais que podem chegar inválidos de fontes externas. Esta parte diferencia estados mutuamente exclusivos de combinações de flags, explica validação e mostra por que nomes, valor zero e máscaras fazem parte do contrato.
...
Um `enum` é um tipo de valor com um conjunto de constantes nomeadas e um tipo integral subjacente, `int` por padrão. Ele melhora legibilidade e segurança de tipo em estados finitos, mas não restringe fisicamente o armazenamento aos membros declarados: casts, dados binários e desserialização podem produzir outros números.
...
O valor padrão de qualquer enum tem todos os bits zero, mesmo quando nenhum membro nomeado vale `0`. Por isso, declare um membro como `Desconhecido = 0` ou valide a inicialização quando zero não fizer sentido. Use `Enum.TryParse` para entrada esperadamente falível e `Enum.IsDefined` quando somente membros nomeados são aceitos; para combinações, siga as regras de `[Flags]`.

> **Referência oficial:** [Enumeration types](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/builtin-types/enum)
...
Um enum com `[Flags]` representa um **conjunto de opções combináveis**, e não uma escolha única. Cada opção independente ocupa normalmente um bit diferente (`1`, `2`, `4`, `8`...), permitindo compor valores com `|` e testá-los com `&`. O atributo melhora a intenção e a formatação textual, mas não impede que um cast produza uma combinação inválida; a API ainda precisa validar entrada externa.

| Operação | Sintaxe | Leitura |
|---|---|---|
| Combinar | `atual \| Permissoes.Escrita` | liga o bit de escrita |
| Remover | `atual & ~Permissoes.Escrita` | desliga o bit de escrita |
| Testar todos | `(atual & exigidas) == exigidas` | todas as permissões exigidas estão presentes |
| Testar alguma | `(atual & candidatas) != 0` | ao menos uma candidata está presente |
| Limpar | `Permissoes.Nenhuma` | nenhum bit ligado |
...
Permissoes exigidas = Permissoes.Leitura | Permissoes.Exclusao;
bool possuiTodas = (usuario & exigidas) == exigidas;
```

**Como interpretar o exemplo:** Aqui o `enum` não representa estados mutuamente exclusivos, e sim capacidades que podem coexistir. `Nenhuma = 0` fornece o estado vazio. Valores independentes devem usar potências de dois; nomes compostos, como `Total`, podem combinar membros já declarados. Ao receber um inteiro, texto ou JSON, rejeite bits que não façam parte da máscara permitida.

> **Armadilha frequente:** declarar valores sequenciais (`1`, `2`, `3`, `4`) em um flags enum. O valor `3` já combina os bits `1` e `2`, portanto deixa de representar uma opção independente.

> **Referências oficiais:** [Enumeration types](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/builtin-types/enum), [`FlagsAttribute`](https://learn.microsoft.com/en-us/dotnet/api/system.flagsattribute)
...
Classes e estruturas combinam dados, comportamento, identidade e invariantes. Este capítulo percorre a anatomia de um tipo, o processo de construção, records, builders, igualdade, indexadores e inicializadores. A pergunta central é como fazer cada instância nascer válida e continuar coerente enquanto participa de coleções, APIs e hierarquias.
...
Uma classe é um tipo de referência que combina estado e comportamento. Cada instância possui identidade própria, enquanto membros estáticos pertencem ao tipo. O desenho saudável mantém campos de implementação encapsulados, expõe operações que preservam invariantes e evita transformar propriedades públicas em um simples espelho mutável de todos os campos.

| Membro | Papel típico |
|---|---|
| campo | armazenamento interno do estado |
| propriedade/indexador | acesso controlado ou valor calculado |
| construtor | criação de uma instância válida |
| método | operação e comportamento |
| evento | notificação controlada pelo publicador |
| tipo aninhado | implementação intimamente ligada ao tipo externo |
...
O exemplo simplifica concorrência: `_totalContas++` e a geração de número precisariam de sincronização em uso multithread. A igualdade por `Numero` também pressupõe que ele seja estável e único. Esses detalhes mostram que uma classe não é apenas sintaxe; ela documenta ownership, mutabilidade, identidade e garantias válidas durante todo o ciclo de vida.

> **Referências oficiais:** [Classes](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/types/classes), [Members](https://learn.microsoft.com/en-us/dotnet/csharp/programming-guide/classes-and-structs/members)
...
O construtor pode validar argumentos e delegar para outro construtor, mas não deve iniciar trabalho assíncrono que o chamador não consiga aguardar.
...
A cópia feita por `with` é rasa: referências internas continuam apontando para os mesmos objetos. Em hierarquias de `record class`, a igualdade também considera o tipo de runtime. Use classe comum quando identidade de instância for central; use record quando o conjunto de valores define naturalmente o conceito.

> **Referência oficial:** [Records](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/types/records)
...
Polimorfismo permite que consumidores trabalhem com um contrato e recebam comportamentos especializados. Esta parte distingue herança de implementação, interfaces de capacidade e composição. O foco é preservar substituibilidade e invariantes: reutilizar código não basta para justificar uma relação “é um”, e abstrações pequenas costumam evoluir melhor.
...
C# trata comportamento como valor por meio de delegates. Lambdas fornecem uma forma compacta de construir esse comportamento; eventos controlam publicação e assinatura; closures capturam contexto; árvores de expressão representam código como dados. Juntos, esses recursos sustentam callbacks, LINQ, APIs reativas e muitos padrões de extensibilidade.

---

### 13.1 Delegates — referências de método tipadas
...
Métodos de instância, métodos estáticos, lambdas e funções locais podem ser convertidos quando parâmetros e retorno são compatíveis. O delegate armazena o método e, para membro de instância, o objeto-alvo. Invocá-lo chama o comportamento de forma tipada; `Method` e `Target` permitem inspeção limitada quando realmente necessária.
...
Em multicast, os métodos são chamados na ordem da lista de invocação. Se um deles lançar, os posteriores normalmente não são chamados. Para delegate com retorno, o resultado observado é o do último método; por isso multicast é mais natural para notificações `void`. Combinar e remover delegates cria novas instâncias imutáveis da lista.

> **Referências oficiais:** [Delegates](https://learn.microsoft.com/en-us/dotnet/csharp/programming-guide/delegates/), [Using delegates](https://learn.microsoft.com/en-us/dotnet/csharp/programming-guide/delegates/using-delegates)
...
Em `Func<...>`, o último argumento de tipo é sempre o retorno e os anteriores são parâmetros. `Action<...>` retorna `void`. `Predicate<T>` representa um teste booleano de um item; muitas APIs modernas usam o tipo equivalente `Func<T, bool>`. Escolha um delegate nomeado quando a operação tem semântica de domínio que merece aparecer no contrato.
...
Esses tipos descrevem apenas a assinatura, não pureza, custo, thread safety ou sincronismo. Um `Func<Task<T>>` representa uma função que inicia/devolve operação assíncrona; é diferente de `Task<Func<T>>`. Evite `Action` para trabalho assíncrono quando isso produzir `async void` sem um contrato de evento.

> **Referências oficiais:** [`Func<TResult>`](https://learn.microsoft.com/en-us/dotnet/api/system.func-1), [`Action<T>`](https://learn.microsoft.com/en-us/dotnet/api/system.action-1), [`Predicate<T>`](https://learn.microsoft.com/en-us/dotnet/api/system.predicate-1)
...
Somente o tipo que declara um evento semelhante a campo pode dispará-lo; consumidores recebem apenas as operações `+=` e `-=`. A convenção .NET usa `EventHandler` ou `EventHandler<TEventArgs>`, `sender` como origem e um tipo derivado de `EventArgs` para dados. Eventos descrevem fatos já ocorridos, portanto nomes no passado ou de mudança comunicam melhor o contrato.
...
Um handler lento bloqueia o disparo síncrono e uma exceção interrompe os handlers seguintes. Se a notificação precisa de isolamento, paralelismo, persistência ou confirmação, um evento CLR simples talvez não seja o mecanismo correto. Para eventos customizados com `add`/`remove`, documente thread safety e evite executar código externo enquanto mantém locks internos.

> **Referências oficiais:** [Events](https://learn.microsoft.com/en-us/dotnet/csharp/programming-guide/events/), [Standard .NET event patterns](https://learn.microsoft.com/en-us/dotnet/csharp/event-pattern)
...
LINQ descreve pipelines tipados de filtro, projeção, agrupamento e agregação. O mesmo vocabulário pode operar sobre objetos locais ou ser traduzido por um provider. Por isso, este capítulo enfatiza fonte, execução adiada, materialização e fronteiras entre `IEnumerable<T>` e `IQueryable<T>`, onde semântica e custo podem mudar.
...
A mesma expressão lambda pode, portanto, ser executada como delegate local ou transformada em uma árvore que outro provider traduz. Sempre identifique a fonte e o ponto de materialização antes de avaliar custo e comportamento.
...
“Adiada” descreve quando a enumeração começa; não significa necessariamente processamento elemento a elemento sem buffer. `OrderBy`, por exemplo, precisa conhecer os elementos para ordená-los antes de produzir a sequência ordenada.

Já uma operação imediata consome a fonte no próprio ponto da chamada e devolve seu resultado naquele momento.
...
Uma coleção comunica como dados podem ser acessados, modificados e compartilhados. Esta parte relaciona contratos a implementações e compara listas, dicionários, arrays, conjuntos e coleções especializadas. Escolher bem exige considerar ordem, unicidade, chave, índice, complexidade, mutabilidade, concorrência e a menor superfície que uma API precisa expor.
...
Uma matriz retangular possui um único objeto com dimensões consultadas por `GetLength`; um jagged guarda arrays independentes e cada linha pode ter tamanho diferente. Expressões de coleção são *target-typed*: `[]` não possui um tipo isolado e o destino determina se será array, `List<T>`, span ou outro tipo suportado. O elemento spread `..` enumera e incorpora uma sequência.
...
Assincronia permite que uma operação fique pendente sem ocupar uma thread apenas esperando, enquanto paralelismo busca executar trabalho simultaneamente. Este capítulo constrói o modelo de `Task`, `await`, cancelamento, composição, streams assíncronos e descarte. A distinção evita bloqueios, `Task.Run` desnecessário e APIs difíceis de cancelar ou observar.
...
Programação assíncrona separa o tempo em que uma operação está pendente do tempo em que uma thread executa código. Um método `async` normalmente executa de forma síncrona até encontrar um `await` incompleto; então registra a continuação e devolve ao chamador uma `Task`, `Task<T>`, `ValueTask` ou outro tipo awaitable permitido.
...
`async` não torna automaticamente uma operação escalável: a API interna também precisa oferecer I/O assíncrono real. Para trabalho CPU-bound, use paralelismo somente quando medido e adequado. Exceções ficam armazenadas na task e são propagadas pelo `await`; por isso tasks não observadas e métodos `async void` fora de event handlers prejudicam diagnóstico.

> **Referências oficiais:** [Asynchronous programming scenarios](https://learn.microsoft.com/en-us/dotnet/csharp/asynchronous-programming/async-scenarios), [Async return types](https://learn.microsoft.com/en-us/dotnet/csharp/asynchronous-programming/async-return-types)
...
Inicie operações independentes antes de aguardá-las para permitir concorrência, mas imponha limites quando a quantidade vier de entrada ou coleção grande. `Task.WhenAll` conclui quando todas terminam; `Task.WhenAny` identifica a primeira concluída e não cancela as restantes. Cancelamento é cooperativo: o token expressa uma solicitação, não uma interrupção forçada.
...
Propague o `CancellationToken` até as APIs internas e preserve `OperationCanceledException` quando o cancelamento pertence ao token esperado. Defina timeout separadamente ou com uma fonte vinculada. `ConfigureAwait(false)` é uma decisão comum de bibliotecas que não precisam retornar ao contexto capturado; não é ritual obrigatório em ASP.NET Core ou em todo aplicativo moderno.

> **Referências oficiais:** [Task asynchronous programming model](https://learn.microsoft.com/en-us/dotnet/csharp/asynchronous-programming/task-asynchronous-programming-model), [Cancellation in managed threads](https://learn.microsoft.com/en-us/dotnet/standard/threading/cancellation-in-managed-threads), [`Task.WhenAll`](https://learn.microsoft.com/en-us/dotnet/api/system.threading.tasks.task.whenall)
...
`Task` é uma referência estável, fácil de armazenar, compartilhar, combinar e aguardar novamente. `ValueTask` é uma struct que pode conter um resultado imediato ou envolver uma operação; foi criada para caminhos quentes nos quais a conclusão síncrona é comum e a alocação de `Task` foi medida como relevante.
...
Não exponha `ValueTask` apenas porque o método “parece rápido”: a struct é maior, aumenta complexidade e pode piorar o caminho assíncrono. Use-a sobretudo ao implementar APIs de baixo nível que documentam as restrições, como `IValueTaskSource`. Para cache de resultado constante, uma `Task` já concluída também pode ser reutilizada.

> **Referência oficial:** [Generalized async return types and `ValueTask<TResult>`](https://learn.microsoft.com/en-us/dotnet/csharp/asynchronous-programming/async-return-types#generalized-async-return-types-and-valuetasktresult)
...
`Repositorio<Usuario>` é um **tipo construído fechado**: todos os parâmetros receberam argumentos concretos e ele pode ser instanciado. `Repositorio<T>` dentro da declaração ainda contém um parâmetro; a forma aberta `Repositorio<>` pode aparecer em APIs de metadados e reflection. Instanciações diferentes, como `Repositorio<Usuario>` e `Repositorio<Produto>`, são tipos distintos, mesmo que compartilhem a mesma definição genérica.
...
Métodos também podem declarar seus próprios parâmetros de tipo, e o compilador frequentemente os infere a partir dos argumentos: `T Identidade<T>(T valor) => valor;`. A inferência não considera apenas o tipo de retorno desejado. Quando a intenção não estiver clara, informe o argumento explicitamente. Prefira nomes semânticos (`TKey`, `TResult`) quando houver mais de um parâmetro; `T` é adequado para um único papel óbvio.
...
Exceções transportam falhas pelo stack até um ponto capaz de tratá-las. Esta parte explica seleção de handlers, filtros, `finally`, preservação do stack trace, hierarquias e exceções de domínio. A meta é distinguir recuperação de ocultação: capturar só quando há uma decisão útil e manter contexto suficiente para diagnóstico.
...
Exceções representam falhas que interrompem o caminho normal. O runtime procura um `catch` compatível do ponto de lançamento para cima na pilha; blocos mais específicos devem aparecer antes dos gerais. Um filtro `when` decide se o handler é aplicável **antes** de o stack ser desenrolado, o que preserva melhor o estado para diagnóstico.
...
try
{
    int divisor = 0;             // variável: a falha ocorre em runtime
...
catch (OverflowException) when (modoEstrito)
{
    // 'when': este handler só participa quando a condição é verdadeira
...
Capture apenas quando puder recuperar, converter para um contrato de fronteira ou adicionar contexto útil. Não use `catch (Exception)` para esconder uma falha e continuar com estado desconhecido. `finally` é executado no desenrolamento normal da pilha, mas não é garantia contra encerramento abrupto do processo. Para liberar recursos, `using` e `await using` expressam melhor o ownership.

> **Referências oficiais:** [Exceptions and exception handling](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/exceptions/), [Exception-handling statements](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/statements/exception-handling-statements), [Exception filters](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/keywords/when#when-in-a-catch-statement)
...
Crie uma exceção própria quando o chamador precisar distinguir uma falha estável do domínio ou consultar dados estruturados sobre ela. Um nome diferente sem comportamento ou contexto adicional normalmente só aumenta a hierarquia. Derive da classe padrão mais específica que preserve a semântica; para uma operação impossível no estado atual, `InvalidOperationException` costuma ser uma base melhor que `Exception`.
...
Mantenha propriedades de contexto imutáveis, preserve uma causa original em `InnerException` quando envolver outra falha e evite dados secretos na mensagem ou no `ToString()`. Documente as exceções que fazem parte de uma API pública. O sufixo `Exception` é a convenção do .NET; `ApplicationException` não é a base recomendada para exceções de aplicação modernas.

> **Referências oficiais:** [How to create user-defined exceptions](https://learn.microsoft.com/en-us/dotnet/standard/exceptions/how-to-create-user-defined-exceptions), [Best practices for exceptions](https://learn.microsoft.com/en-us/dotnet/standard/exceptions/best-practices-for-exceptions), [Designing custom exceptions](https://learn.microsoft.com/en-us/dotnet/standard/design-guidelines/designing-custom-exceptions)
...
Toda exceção capturável deriva de `System.Exception`, mas o tipo concreto informa a categoria da falha e orienta o tratamento. Exceções de argumento apontam violação do contrato de entrada; `InvalidOperationException` aponta estado inadequado; `IOException` agrupa falhas de I/O. Capturar uma base ampla perde essa precisão e pode misturar problemas recuperáveis com defeitos de programação.
...
│   ├── StackOverflowException    — esgotamento da pilha; normalmente encerra o processo
│   ├── OutOfMemoryException      — falha de alocação; recuperação confiável é rara
...
Não decida o tratamento pelo texto de `Message`: ele não é um contrato estável e pode variar. Use o tipo, propriedades documentadas e, quando necessário, filtros. Algumas falhas de runtime, como stack overflow, não oferecem um caminho confiável de recuperação no processo. Registre contexto nos limites da aplicação, mas evite registrar a mesma exceção repetidamente em cada camada.

> **Referências oficiais:** [System.Exception](https://learn.microsoft.com/en-us/dotnet/api/system.exception), [Exceptions and exception handling](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/exceptions/), [Best practices for exceptions](https://learn.microsoft.com/en-us/dotnet/standard/exceptions/best-practices-for-exceptions)
...
Attributes gravam metadados declarativos para compiladores, ferramentas, reflection e bibliotecas. Este capítulo mostra atributos embutidos, contratos customizados e informações inseridas sobre o chamador. A chave é lembrar que metadado não executa comportamento sozinho: algum consumidor precisa reconhecer e interpretar o atributo de forma documentada.
...
O sufixo `Attribute` pode ser omitido no uso: `[Obsolete]` representa `ObsoleteAttribute`. Quando o alvo for ambíguo, prefixe-o, como `[return: MeuAtributo]`, `[field: NonSerialized]` ou `[assembly: InternalsVisibleTo("Testes")]`. Prefira atributos para informação declarativa estável; configuração operacional que muda por ambiente normalmente pertence a opções, arquivos ou serviços de configuração.

> **Referências oficiais:** [Attributes](https://learn.microsoft.com/en-us/dotnet/csharp/advanced-topics/reflection-and-attributes/), [Common attributes](https://learn.microsoft.com/en-us/dotnet/csharp/advanced-topics/reflection-and-attributes/#common-attributes), [Attribute targets](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/attributes/general)
...
Um atributo customizado deriva de `System.Attribute`. `[AttributeUsage]` restringe onde ele pode aparecer, se pode se repetir no mesmo alvo e se é herdado. Argumentos do construtor são posicionais e devem ser representáveis nos metadados; propriedades ou campos públicos graváveis fornecem argumentos nomeados opcionais.
...
O valor de um argumento de atributo precisa ser conhecido em compilação e pertencer aos tipos permitidos, como primitivos, `string`, `Type`, `enum`, `object` desses tipos ou arrays unidimensionais correspondentes. Ler atributos com reflection pode instanciar as classes de atributo; ferramentas que só precisam dos metadados podem usar `CustomAttributeData`. Defina um contrato pequeno, documente quem o consome e teste alvos inválidos.

> **Referências oficiais:** [Create custom attributes](https://learn.microsoft.com/en-us/dotnet/csharp/advanced-topics/reflection-and-attributes/creating-custom-attributes), [AttributeUsageAttribute](https://learn.microsoft.com/en-us/dotnet/api/system.attributeusageattribute), [Access attributes using reflection](https://learn.microsoft.com/en-us/dotnet/csharp/advanced-topics/reflection-and-attributes/accessing-attributes-by-using-reflection)

---

### 19.3 Informações do chamador e atributos modernos

[⬆️ Voltar ao Sumário](#sumário)

Os atributos de informação do chamador pedem ao compilador que preencha parâmetros opcionais com dados do ponto de chamada. Eles são úteis para logging, diagnóstico, validação e implementação de `INotifyPropertyChanged`, sem depender de inspeção de pilha. O chamador ainda pode fornecer esses argumentos explicitamente, então não os trate como mecanismo de segurança.
...
static void Exigir(
    bool condicao,
    [CallerArgumentExpression(nameof(condicao))] string? expressao = null,
    ...
    [CallerLineNumber] int linha = 0)
{
    if (!condicao)
        throw new ArgumentException(
            $"Falhou: {expressao}; membro={membro}; arquivo={Path.GetFileName(arquivo)}; linha={linha}");
}
...
> **Referências oficiais:** [Caller information](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/attributes/caller-information), [Attributes — generic attributes](https://learn.microsoft.com/en-us/dotnet/csharp/advanced-topics/reflection-and-attributes/#generic-attributes), [ModuleInitializerAttribute](https://learn.microsoft.com/en-us/dotnet/api/system.runtime.compilerservices.moduleinitializerattribute)
...
Esta parte reúne ferramentas que resolvem problemas específicos de representação e memória: tuplas, referências fracas, spans, resultados modelados por hierarquias e o GC. Elas não formam uma única família de sintaxe; o elo é exigir uma decisão explícita sobre identidade, cópia, tempo de vida, ownership e custo.
...
As tuplas modernas usam `System.ValueTuple`: são tipos de valor com campos públicos e sintaxe especial da linguagem. Os nomes `Nome` e `Idade` melhoram a leitura e são preservados como metadados para ferramentas, mas não fazem parte da identidade estrutural do tipo. Assim, `(int X, int Y)` e `(int Largura, int Altura)` têm os mesmos tipos de elementos e podem ser atribuídos entre si.
...
Tuplas oferecem igualdade por valor, desconstrução e acesso por nome ou por `Item1`, `Item2` etc. Use `_` para descartar partes que não interessam. Elas funcionam bem como resultados locais e temporários; para contratos públicos duradouros, validação, documentação própria ou comportamento, prefira `record`, `record struct`, classe ou estrutura nomeada. Não confunda essa sintaxe com a classe histórica `System.Tuple`, que é um tipo de referência.

> **Referências oficiais:** [Tuple types](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/builtin-types/value-tuples), [Deconstructing tuples](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/functional/deconstruct), [Choose between anonymous types and tuples](https://learn.microsoft.com/en-us/dotnet/standard/base-types/choosing-between-anonymous-and-tuple)
...
Use `ReadOnlySpan<T>` para entrada síncrona quando o método não precisa reter a região; use `ReadOnlyMemory<T>` quando o dado precisa sobreviver ao retorno, ser armazenado ou atravessar `await`. A abstração não possui ownership: quem fornece a memória continua responsável por seu tempo de vida e por evitar mutação concorrente. Comece com arrays e strings; adote spans em caminhos medidos onde evitar cópias e substrings traz benefício real.
...
Esse desenho torna estados inválidos menos representáveis: em vez de combinar `bool`, valor nulo e mensagem opcional, o resultado é exatamente um caso nomeado. O custo é aumentar o número de tipos e depender de uma hierarquia; para APIs simples, `TryXxx`, exceções ou um resultado genérico reutilizável podem comunicar melhor. Como C# 14 não fecha hierarquias para análise exaustiva, mantenha o braço de fallback e faça-o falhar de forma visível quando um caso novo deveria ser tratado.

> **Referências oficiais:** [Records](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/types/records), [Pattern matching](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/functional/pattern-matching), [`switch` expression](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/operators/switch-expression)
...
Concorrência permite progresso de trabalhos sobrepostos; paralelismo executa trabalho ao mesmo tempo. Esta parte vai de threads e pool à TPL, sincronização, visibilidade de memória, coleções concorrentes e canais. O objetivo central é proteger invariantes e limitar contenção, preferindo imutabilidade e isolamento quando eliminam estado compartilhado.
...
Uma `Thread` é um fluxo de execução do sistema operacional com pilha e estado próprios. Criá-la diretamente dá controle sobre prioridade, modo foreground/background e tempo de vida, mas custa mais que reutilizar o pool. Threads foreground mantêm o processo vivo; threads background não impedem seu encerramento. A maioria das aplicações deve trabalhar com `Task`, `async` e primitivas de nível mais alto.
...
O `ThreadPool` reutiliza workers para tarefas curtas e não oferece um vínculo estável entre trabalho e thread. Bloquear muitos workers com espera síncrona pode causar **thread-pool starvation**, aumentar latência e atrasar continuações. Use APIs assíncronas para I/O; reserve threads dedicadas para casos especializados, como integração que exige afinidade ou uma operação longa e bloqueante que não possui alternativa assíncrona.

> **Referências oficiais:** [Threads and threading](https://learn.microsoft.com/en-us/dotnet/standard/threading/threads-and-threading), [The managed thread pool](https://learn.microsoft.com/en-us/dotnet/standard/threading/the-managed-thread-pool), [Foreground and background threads](https://learn.microsoft.com/en-us/dotnet/standard/threading/foreground-and-background-threads)
...
A TPL representa unidades de trabalho com `Task` e oferece particionamento, agendamento, cancelamento e agregação de resultados. Uma task não é necessariamente uma nova thread: o scheduler normalmente usa o `ThreadPool`, e uma task de I/O assíncrono pode passar a maior parte do tempo sem executar em thread alguma. `Task.Run` serve principalmente para deslocar trabalho CPU-bound para o pool, não para embrulhar uma API de I/O já assíncrona.
...
`Parallel.For`, `Parallel.ForEach` e PLINQ podem acelerar trabalho CPU-bound independente, mas não preservam ordem por padrão e podem piorar tarefas pequenas por overhead. O corpo precisa ser thread-safe; evite escrever numa coleção comum compartilhada. Controle cancelamento e grau de paralelismo, observe todas as exceções e meça com carga real. Paralelismo aumenta throughput possível, mas também aumenta consumo simultâneo de CPU, memória e dependências.

> **Referências oficiais:** [Task Parallel Library](https://learn.microsoft.com/en-us/dotnet/standard/parallel-programming/task-parallel-library-tpl), [Data parallelism](https://learn.microsoft.com/en-us/dotnet/standard/parallel-programming/data-parallelism-task-parallel-library), [PLINQ](https://learn.microsoft.com/en-us/dotnet/standard/parallel-programming/introduction-to-plinq)
...
Quando vários fluxos acessam estado mutável compartilhado, uma sequência aparentemente simples pode intercalar. `contador++`, por exemplo, envolve leitura, soma e escrita; duas threads podem ler o mesmo valor e perder uma atualização. **Sincronização** protege a invariável do estado e também estabelece regras de visibilidade de memória entre threads.

| Primitiva | Use quando | Cuidado principal |
|---|---|---|
| `lock` / `Monitor` | uma região síncrona precisa de exclusão mútua | não use `await` dentro do bloco |
| `Interlocked` | uma operação atômica simples sobre um campo | não compõe várias invariantes sozinho |
| `SemaphoreSlim` | é preciso limitar concorrência, inclusive com `WaitAsync` | libere em `finally` |
| `ReaderWriterLockSlim` | muitas leituras e poucas escritas, após medição | maior complexidade e risco de contenção |
| `Mutex` | sincronização entre processos | custo maior que primitivas apenas em processo |

O objeto usado como gate deve ser privado e dedicado. Não bloqueie `this`, `typeof(T)` nem uma `string`, pois outro código pode usar a mesma referência sem você saber. Mantenha a região crítica curta e proteja **todo** o conjunto de dados que forma a invariável.
...
**Como interpretar o exemplo:** `_contador++` precisa do `lock` porque é uma operação composta. Para apenas incrementar ou trocar um campo, `Interlocked` expressa melhor a atomicidade. Já `SemaphoreSlim` não protege automaticamente um objeto: ele controla quantos consumidores entram e é útil quando o recurso tolera um número limitado de operações simultâneas.

> **Armadilha frequente:** adquirir uma primitiva e retornar ou lançar antes de liberá-la. Use `lock`, `using` quando a API fornecer um escopo descartável, ou `try/finally` como no exemplo do semáforo.

> **Referências oficiais:** [`lock` statement](https://learn.microsoft.com/en-us/dotnet/csharp/language-reference/statements/lock), [`Interlocked`](https://learn.microsoft.com/en-us/dotnet/api/system.threading.interlocked), [`SemaphoreSlim`](https://learn.microsoft.com/en-us/dotnet/api/system.threading.semaphoreslim), [Overview of synchronization primitives](https://learn.microsoft.com/en-us/dotnet/standard/threading/overview-of-synchronization-primitives)
...
Aplicações reais atravessam fronteiras: tipos descobertos dinamicamente, objetos compostos por DI, código gerado no build e bibliotecas nativas. Este capítulo explica essas ferramentas e seus custos de segurança, desempenho e compatibilidade. Reflection e `unsafe` são poderosos, mas devem ficar atrás de contratos tipados e estreitos sempre que possível.
...
O gerador recebe uma visão do código e de entradas adicionais durante o build; ele não é um plugin que fica executando dentro da aplicação. O resultado deve ser determinístico para as mesmas entradas e pode ser inspecionado como qualquer outro código gerado. **Analisadores** produzem diagnósticos e code fixes; **source generators** adicionam fontes. Um mesmo pacote Roslyn pode oferecer ambas as capacidades.
...
Não gere código a partir de estado externo mutável, relógio ou rede: isso prejudica builds incrementais e reproduzíveis. Use diagnósticos para entradas inválidas, nomes estáveis para arquivos gerados e namespaces que não colidam. Geração pode melhorar compatibilidade com trimming e AOT ao substituir descoberta por reflection, mas só quando o código produzido realmente descreve os membros necessários.

> **Referências oficiais:** [Source generators](https://learn.microsoft.com/en-us/dotnet/csharp/roslyn-sdk/source-generators-overview), [Incremental generators cookbook](https://github.com/dotnet/roslyn/blob/main/docs/features/incremental-generators.cookbook.md), [System.Text.Json source generation](https://learn.microsoft.com/en-us/dotnet/standard/serialization/system-text-json/source-generation)
...
Código comum em C# é verificavelmente seguro: o runtime controla referências, limites e tempo de vida. Um contexto `unsafe` habilita ponteiros e operações cuja segurança as ferramentas não conseguem provar. Isso não torna o trecho automaticamente incorreto, mas transfere para o autor a responsabilidade por endereços válidos, limites, alinhamento, pinning e ownership. O projeto precisa habilitar `AllowUnsafeBlocks`.
...
O GC pode mover objetos gerenciados; `fixed` fixa temporariamente a região para obter um ponteiro estável. Mantenha o pinning curto, pois ele restringe a compactação do heap. Um ponteiro não carrega tamanho nem nulabilidade segura, e não pode escapar do tempo de vida da memória apontada. Prefira `Span<T>`, `Memory<T>`, `MemoryMarshal` e APIs de interop geradas quando elas expressarem o mesmo trabalho com garantias melhores.

Ponteiros de função (`delegate*`) representam endereços chamáveis sem criar um delegate e exigem assinatura e calling convention corretas. **Inline arrays**, introduzidos no C# 12 por `[InlineArray(N)]`, armazenam um número fixo de elementos dentro de uma estrutura e são úteis em interop e buffers de baixo nível; não são substitutos gerais de `T[]` ou `Span<T>`.
...
{
    private byte _element0; // o compilador projeta os demais elementos contíguos
}

public static unsafe class OperacaoBaixoNivel
{
    // Exemplo de campo que receberia o endereço de uma função nativa compatível.
    private static delegate* unmanaged[Cdecl]<byte*, int, int> _processar;
}
```
...
A linguagem continua sendo C#, mas engines definem ciclo de vida, serialização, threads, build e APIs próprias. Esta parte conecta os fundamentos ao Unity e ao Godot sem tratar callbacks da engine como recursos da linguagem. Consulte sempre a documentação da versão da engine, especialmente para runtime, plataformas, AOT e suporte de C#.
...
Campos serializados pertencem ao contrato com cenas e prefabs: renomeá-los ou mudar seu tipo pode afetar dados já gravados. Evite construtores de `MonoBehaviour`; inicialize conforme o ciclo oficial e mantenha a lógica de domínio em classes C# comuns quando ela não depender da engine.

> **Referências oficiais Unity:** [MonoBehaviour](https://docs.unity3d.com/ScriptReference/MonoBehaviour.html), [Script serialization](https://docs.unity3d.com/Manual/script-serialization.html)
...
Unity invoca mensagens de ciclo de vida quando certas condições da cena e do componente são atendidas; não são overrides de uma interface C# comum. `Awake` é apropriado para inicialização interna, `OnEnable` para inscrições que devem existir enquanto o componente está habilitado e `Start` para trabalho antes do primeiro update. A ordem entre objetos diferentes exige configuração ou design explícito; não dependa casualmente da ordem visual da Hierarchy.
...
`FixedUpdate` pode executar zero, uma ou várias vezes antes de um frame renderizado; use-o para interação com física, não como relógio universal. `Update` recebe a cadência dos frames e normalmente usa `Time.deltaTime`. Libere inscrições e recursos em `OnDisable` ou `OnDestroy` conforme o mesmo escopo em que foram adquiridos.

> **Referência oficial Unity:** [Order of execution for event functions](https://docs.unity3d.com/Manual/execution-order.html)
...
Interromper o `MonoBehaviour` ou destruir o objeto afeta o tempo de vida das coroutines conforme as regras da engine; modele cancelamento explicitamente quando o resultado importar. Objetos como `WaitForSeconds` também podem gerar alocações quando criados repetidamente. Não coloque CPU pesada dentro da coroutine: até o próximo `yield`, o código continua executando na thread principal e pode travar o frame.

> **Referência oficial Unity:** [Coroutines](https://docs.unity3d.com/Manual/Coroutines.html)
...
`UnityEvent` é serializável e permite que conexões persistentes sejam configuradas no Inspector, o que favorece composição por designers e cenas. Um `event` normal de C# é conhecido pelo compilador, inscrito por código e costuma ser a opção mais direta para comunicação interna fortemente tipada. Ambos representam notificação; o mecanismo de configuração, serialização e diagnóstico é diferente.
...
Emparelhe inscrição e cancelamento no mesmo escopo, normalmente `OnEnable`/`OnDisable`. Uma referência mantida pelo publicador pode prolongar a vida lógica do assinante e causar callbacks duplicados após reativação. Para eventos frequentes e internos, avalie custo e clareza; para conexões editáveis, documente no componente quais callbacks são esperados no Inspector.

> **Referências oficiais:** [UnityEvents](https://docs.unity3d.com/Manual/UnityEvents.html), [C# events](https://learn.microsoft.com/en-us/dotnet/csharp/events-overview)
...
Padrões dão nomes a soluções recorrentes, mas não substituem análise de ownership, ciclo de vida e dependências. Em jogos, o loop por frame e a criação/destruição de objetos tornam esses custos visíveis rapidamente. Comece pelo problema: estado variável favorece State, notificação para vários interessados favorece Observer e reutilização de objetos caros ou frequentes pode favorecer Pooling.
...
Singleton introduz acesso global e deve ter ownership e reset entre cenas/testes bem definidos. State transforma condicionais espalhadas em transições explícitas, mas pode produzir classes demais para uma máquina simples. Um event bus reduz referências diretas, porém esconde o fluxo quando todos publicam tudo globalmente. Prefira escopos menores, nomes de evento específicos e medições antes de aplicar pooling ou outras otimizações.

> **Referências oficiais:** [Unity object pooling](https://docs.unity3d.com/ScriptReference/Pool.ObjectPool_1.html), [C# events](https://learn.microsoft.com/en-us/dotnet/csharp/events-overview)
...
Scripts C# do Godot são classes `partial` porque o source generator da engine cria partes necessárias à integração. O scene tree define ownership e ciclo de vida; callbacks como `_Ready`, `_Process` e `_PhysicsProcess` pertencem a esse contrato, não à linguagem C#. Signals são o mecanismo de eventos da engine e também podem ser conectados pela API C#.
...
Use as APIs de `GodotObject` e respeite o tempo de vida controlado pela engine; uma referência gerenciada não garante que o objeto nativo continue válido. Muitos objetos da engine têm afinidade com a thread principal. Para publicar, confira as plataformas suportadas pela edição .NET da versão escolhida, pois elas não são necessariamente as mesmas da edição padrão.

> **Referências oficiais Godot:** [C# basics](https://docs.godotengine.org/en/stable/tutorials/scripting/c_sharp/c_sharp_basics.html), [C# signals](https://docs.godotengine.org/en/stable/tutorials/scripting/c_sharp/c_sharp_signals.html), [C# platform support](https://docs.godotengine.org/en/stable/tutorials/scripting/c_sharp/index.html)
...
Arquitetura também precisa refletir implantação e equipe. Um monólito modular pode oferecer limites excelentes com operação simples; microservices podem ser corretos quando partes realmente precisam de autonomia de deploy e escala. Use namespaces, assemblies, projetos e testes de arquitetura para tornar limites intencionais verificáveis, sem confundir quantidade de pastas com qualidade do desenho.
...
Entidades possuem identidade ao longo do tempo; value objects são definidos pelos valores; aggregates delimitam consistência transacional e acesso por uma raiz. Bounded contexts reconhecem que o mesmo termo pode ter modelos diferentes em partes distintas da organização. Essas fronteiras são decisões de domínio primeiro e só depois viram classes, projetos, bancos ou serviços.
...
Na forma mais simples, comandos e consultas podem compartilhar banco e processo, diferenciando apenas contratos e responsabilidades. Separar modelos físicos, usar mensageria ou event sourcing aumenta consistência eventual e operação; só adote essas extensões quando uma necessidade mensurável justificar o custo. Um comando deve expressar intenção e uma query não deve produzir efeitos observáveis.
...
Eventos internos de domínio coordenam objetos no mesmo limite transacional; eventos de integração atravessam processos e exigem um contrato versionado. Entrega de mensagens costuma ser ao menos uma vez, portanto consumidores precisam tolerar repetição. O padrão outbox pode coordenar persistência e publicação, mas também adiciona infraestrutura, retenção e monitoramento que precisam ser operados.
...
DTO não é entidade, e Service Layer não precisa virar uma classe genérica que apenas encaminha chamadas. Defina as fronteiras onde dados, transação ou linguagem realmente mudam. Em EF Core, `DbContext` já combina comportamentos de Unit of Work e Repository; uma abstração adicional deve expressar operações do domínio, facilitar um limite real ou isolar uma dependência — não duplicar cada método da ferramenta.
...
Este capítulo conecta esses papéis a projetos, dependências, build, testes, empacotamento, analisadores e documentação. A meta é sair do “funciona na minha máquina” para um processo versionado que outro ambiente consiga restaurar, verificar e publicar com os mesmos contratos.
...
Essas etapas formam um pipeline, mas produzem resultados diferentes. Restore resolve o grafo; build transforma fontes e referências em assemblies; test executa verificações; pack cria uma biblioteca distribuível; publish monta uma aplicação pronta para um destino. Saber qual artefato está sendo validado evita testes verdes sobre uma saída diferente da que será implantada.
...
Execute restore bloqueado quando o repositório usa lock file e trate warnings do build e da publicação como sinais distintos: trimming, single-file e AOT têm analisadores próprios. Faça smoke test do diretório publicado no mesmo sistema operacional e arquitetura do destino. Preserve logs, resultados de teste, símbolos e hashes necessários para diagnosticar e reproduzir uma versão.
...
Analisadores examinam o código e o grafo sem precisar executar a aplicação. Regras do compilador verificam a linguagem; analisadores de qualidade procuram padrões de corretude, segurança, performance e design; regras de estilo alinham forma. Diferencie essas categorias ao definir severidade e mantenha a configuração versionada junto do fonte.
...
O capítulo aplica esse modelo a streams, JSON, HTTP, cultura, datas e expressões regulares, sempre separando formato externo de regra de negócio e indicando limites de segurança e compatibilidade.
...
As opções pertencem ao contrato de serialização e devem ser reutilizadas depois de configuradas; mutá-las durante uso concorrente é um erro de desenho. `JsonSerializerDefaults.Web` aplica convenções comuns de aplicações web, mas atributos no DTO podem tornar decisões importantes locais e explícitas. Diferencie ausência de propriedade, `null` e valor padrão quando isso tiver significado para compatibilidade.
...
Conversores customizados resolvem formatos que o contrato padrão não representa, mas passam a ser parte crítica de segurança e versionamento. Não desserialize tipos polimórficos arbitrários de entrada externa. Source generation torna metadados conhecidos em build e reduz dependência de reflection; ainda exige registrar todos os tipos e opções realmente usados.
...
Timeout total, cancelamento do usuário e limites de uma etapa podem ter causas distintas; preserve essa distinção no diagnóstico. Faça streaming de corpos grandes e imponha limites. Uma resposta HTTP de erro não é necessariamente exceção de transporte, e uma resposta de sucesso ainda pode conter um payload inválido para seu contrato.
...
Prefira `[GeneratedRegex]` para padrões conhecidos em compilação e reutilizados; para padrões dinâmicos, reutilize a instância quando apropriado e sempre controle o timeout. Capture apenas os grupos necessários e teste entradas adversariais, não só exemplos válidos. Regex reconhece linguagem textual; parsers dedicados são melhores para formatos aninhados, contextuais ou com gramática complexa.
...
Esta parte transforma essas qualidades em práticas verificáveis: estratégia de testes, logging estruturado, opções validadas, diagnóstico, proteção de dados, publicação e evolução de APIs. O fio condutor é tornar falhas visíveis e mudanças deliberadas antes que cheguem ao usuário.
...
Configuração é composta por provedores em uma ordem de precedência; documente quais fontes podem sobrescrever cada opção. Valide opções no startup para falhar cedo. Em logging, use escopos e identificadores de correlação quando ajudarem a seguir uma operação, mas controle cardinalidade para não tornar armazenamento e consultas imprevisíveis.
...
O modelo de publicação é uma decisão de compatibilidade, tamanho, startup, atualização e operação. Comece pelo requisito do ambiente: runtime já gerenciado favorece framework-dependent; execução isolada pode pedir self-contained; distribuição simplificada pode pedir single-file; restrições de startup e memória podem justificar Native AOT. Esses recursos são combináveis em alguns cenários, mas não são sinônimos.
...
Trimming depende de análise estática e pode remover membros acessados apenas por reflection, serialização dinâmica ou carregamento de plugins. Resolva warnings com contratos e anotações adequados, não conservando tudo indiscriminadamente. Native AOT proíbe geração de código dinâmica e limita certos usos de reflection; verifique a compatibilidade de cada dependência e teste o artefato publicado no RID real desde o início do projeto.
...
> Termos-chave em ordem alfabética. A coluna “Estude em” leva à explicação completa, com exemplos, limites e referências oficiais.

| Termo | Definição didática | Estude em |
|---|---|---|
...
| **Caller information** | Metadados inseridos pelo compilador sobre membro, arquivo, linha ou expressão do ponto de chamada. | [19.3 Informações do chamador](#193-informações-do-chamador-e-atributos-modernos) |
| **CancellationToken** | Sinal cooperativo que permite solicitar cancelamento sem encerrar forçadamente uma thread ou operação. | [16.2 Padrões de uso](#162-padrões-de-uso) |
| **Clean Architecture** | Organização que protege regras centrais de detalhes externos por meio de dependências orientadas para dentro. | [24.3 Clean Architecture](#243-clean-architecture-onion-e-ports-and-adapters) |
| **Closure** | Lambda ou função local que captura variáveis de seu escopo externo e pode prolongar seu tempo de vida. | [13.5 Closures](#135-closures-e-árvores-de-expressão) |
| **CLR** | Runtime que carrega assemblies, gerencia execução, GC, exceções e, normalmente, compilação JIT. | [1.1 O que é C#?](#11-o-que-é-c) |
| **Collection expression** | Sintaxe `[...]` target-typed para construir coleções e incorporar sequências com spread `..`. | [15.5 Arrays e coleções](#155-arrays-coleções-imutáveis-congeladas-e-concorrentes) |
| **Constraint genérica** | Regra `where` que limita argumentos de tipo e habilita operações conhecidas no corpo genérico. | [17.2 Constraints](#172-constraints-restrições) |
| **Construtor** | Membro sem tipo de retorno que estabelece o estado inicial de uma nova instância. | [11.2 Construtores](#112-construtores-em-profundidade) |
| **`const` / `readonly`** | `const` fixa um valor de compilação; `readonly` restringe reatribuição do campo, não a mutação do objeto referido. | [3.5 `const` e `readonly`](#35-const-e-readonly) |
| **Coroutine** | Rotina cooperativa do Unity que suspende e retoma trabalho ao longo de frames, sem criar paralelismo. | [23.5 Coroutines](#235-coroutines--execução-cooperativa-ao-longo-de-frames) |
| **Covariância / contravariância** | Regras de conversão para interfaces e delegates genéricos produtores (`out`) e consumidores (`in`). | [17.3 Variância](#173-covariância-e-contravariância) |
| **CQRS** | Separação entre operações que mudam estado e consultas que apenas produzem uma leitura. | [24.5 CQRS](#245-cqrs-e-separação-entre-comandos-e-consultas) |
| **Cultura** | Conjunto de convenções para formatar, comparar e interpretar texto, números e datas para uma localidade. | [26.4 Cultura e parsing](#264-cultura-parsing-datas-e-fusos-horários) |
| **Delegate** | Tipo que representa uma chamada de método com assinatura conhecida e pode compor uma lista de callbacks. | [13.1 Delegates](#131-delegates--referências-de-método-tipadas) |
| **Dependency Injection** | Composição em que um objeto recebe dependências em vez de criá-las internamente. | [22.2 Dependency Injection](#222-dependency-injection-di) |
| **Descarte (`_`)** | Marcador de que um valor, parâmetro ou parte de uma desconstrução não será usado naquele contexto. | [3.8 Literais e descartes](#38-literais-target-typing-e-descartes) |
| **Domain-Driven Design** | Abordagem que modela software a partir das regras, fronteiras e linguagem do domínio. | [24.4 DDD](#244-domain-driven-design-ddd) |
| **Enum / Flags enum** | Tipo de valores nomeados; com `[Flags]`, pode representar combinações de bits independentes. | [10.1 Enums](#101-enums-básicos) e [10.2 Flags](#102-flags-enum--bitmask) |
| **Event** | Encapsulamento de um delegate para que assinantes reajam e apenas o publicador dispare a notificação. | [13.4 Eventos](#134-eventos-events) |
...
| **HttpClient** | Cliente HTTP que usa handlers e pools de conexões e deve ter tempo de vida planejado. | [26.3 HTTP](#263-http-e-tempo-de-vida-do-httpclient) |
| **IL / CIL** | Linguagem intermediária gravada em assemblies .NET antes da geração ou execução de código nativo. | [1.1 O que é C#?](#11-o-que-é-c) |
| **`IEnumerable<T>`** | Contrato de enumeração de uma sequência; não promete índice, tamanho barato nem materialização. | [14.3 `IEnumerable<T>`](#143-ienumerablet-e-o-contrato-fundamental-das-sequências) |
| **`IQueryable<T>`** | Consulta representada como expressão que um provider pode traduzir para outra fonte, como SQL. | [14.4 `IQueryable<T>`](#144-iqueryablet-e-queries-traduzíveis-para-outra-fonte) |
| **Interface** | Contrato de capacidades implementável por vários tipos, com suporte moderno a membros padrão e estáticos. | [12.2 Interfaces](#122-interfaces) |
| **JIT** | Compilação de IL para código nativo durante a execução, normalmente feita pelo runtime por método. | [1.1 O que é C#?](#11-o-que-é-c) |
| **Jobs System** | Sistema da Unity para agendar trabalho paralelo compatível com suas regras de dados e segurança. | [23.7 Performance no Unity](#237-boas-práticas-de-performance-no-unity) |
| **Lambda** | Função anônima que pode ser convertida em delegate ou árvore de expressão conforme o tipo alvo. | [13.3 Lambdas](#133-expressões-lambda) |
| **LINQ** | Operadores e sintaxe integrados para consultar e transformar sequências e fontes traduzíveis. | [14.1 LINQ](#141-o-que-é-linq) |
| **Marshalling** | Conversão de representações e ownership ao atravessar a fronteira entre código gerenciado e nativo. | [22.5 Interop nativa](#225-interoperabilidade-nativa-e-marshalling) |
...
| **RID** | Identificador de runtime que descreve sistema operacional e arquitetura de um destino de publicação. | [27.5 Publicação](#275-publicação-trimming-single-file-e-native-aot) |
| **Roslyn** | Compilador e plataforma de análise do C#, usada também por analisadores e source generators. | [1.1 O que é C#?](#11-o-que-é-c) |
| **`sealed`** | Impede herança de uma classe ou nova sobrescrita de um membro virtual. | [7.2 `sealed`](#72-sealed) |
| **ScriptableObject** | Asset Unity para dados serializados independentes de uma instância de `GameObject`. | [23.4 ScriptableObject](#234-scriptableobject--dados-desacoplados-do-gameobject) |
| **Singleton** | Padrão que oferece uma instância compartilhada dentro de um escopo e tempo de vida definidos. | [23.8 Padrões em jogos](#238-padrões-de-design-comuns-em-jogos-com-c) |
| **Source generator** | Componente Roslyn que adiciona fontes C# à compilação de forma determinística. | [22.3 Source Generators](#223-source-generators-c-9) |
| **`Span<T>` / `Memory<T>`** | Visões de memória contígua; `Span<T>` é stack-only, enquanto `Memory<T>` pode ser armazenado e atravessar `await`. | [20.3 Memória e spans](#203-spant-e-memoryt--fatias-de-memória) |
| **`static`** | Faz o membro pertencer ao tipo em vez de a uma instância específica. | [7.1 `static`](#71-static) |
| **State Machine** | Modelo de comportamento com estados possíveis e transições explícitas. | [23.8 Padrões em jogos](#238-padrões-de-design-comuns-em-jogos-com-c) |
| **StringBuilder** | Buffer textual mutável para montagem incremental de texto, especialmente em repetição. | [4.2 StringBuilder](#42-imutabilidade-e-stringbuilder) |
| **Task / ValueTask** | Awaitables que representam conclusão futura; `ValueTask` é uma otimização especializada. | [16.3 Task e ValueTask](#163-task-vs-valuetask) |
| **TFM** | Target Framework Moniker que define a superfície de APIs contra a qual o projeto compila. | [25.2 `.csproj` e TFM](#252-csproj-tfm-e-versão-da-linguagem) |
| **ThreadPool** | Conjunto gerenciado de threads reutilizadas para trabalhos curtos e continuações. | [21.1 Thread e ThreadPool](#211-thread-básico-e-threadpool) |
| **Trimming** | Remoção, na publicação, de código que a análise considera não alcançável. | [27.5 Publicação](#275-publicação-trimming-single-file-e-native-aot) |
| **Unsafe code** | Contexto que permite ponteiros e operações cuja segurança de memória o runtime não consegue verificar. | [22.4 Unsafe](#224-unsafe-code-e-ponteiros) |
| **`var`** | Inferência local estática: o compilador determina um tipo concreto a partir do inicializador. | [3.4 `var`](#34-var--inferência-de-tipo) |
| **`virtual` / `override`** | Declaram um ponto de polimorfismo e a implementação que substitui o comportamento da base. | [7.4 `virtual` e `override`](#74-virtual-e-override) |
| **`WeakReference<T>`** | Referência que permite observar um alvo sem impedir sua coleta quando não houver referências fortes. | [20.2 Referências fracas](#202-weakreferencet-e-referências-fracas-no-gc) |

**Como usar a tabela:** trate o glossário como índice de revisão. As definições são deliberadamente curtas; detalhes de versão, runtime, trade-offs e contratos permanecem nas seções vinculadas.
